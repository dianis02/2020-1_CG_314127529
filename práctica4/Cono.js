  ...
  class Cono {
    ...
    */
    ...
    getVertices(radius, height, Nu,Nv) {
      let pos = buildCone(radius, height, Nu,Nv).vertices;
      return pos;
    }
    ...
    */
    getFaces(radius, height, Nu,Nv) {
    	  let face = buildCone(radius, height, Nu,Nv).faces;
    ...
    */
      ...
      * @return {Array} faces
      */
      function buildCone(radius, height, Nu, Nv, initial_transform) {
        radius = (radius || 1);
        height = (height || 1);
        Nu = Nu || 2;
        Nv = Nv || 2;
        ...
        for (let i=0; i<Nv; i++) {
          for (let j=0; j<Nu; j++) {
              ...
              vertices.push(radius * (Nv-i)/Nv * Math.cos(j*2*Math.PI/Nu));
              vertices.push(-height + i*2*height/Nv);
              vertices.push(radius * (Nv-i)/Nv * Math.sin(j*2*Math.PI/Nu));
    ...
    faces.push(vertices.length-1 -Nu +i);
    faces.push(vertices.length-1 -Nu +(i+1)%Nu);
    }
